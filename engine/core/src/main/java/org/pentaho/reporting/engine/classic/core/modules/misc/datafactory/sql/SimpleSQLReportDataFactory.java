...
public class SimpleSQLReportDataFactory extends AbstractDataFactory {
  ...
  private static final String COLUMN_NAME_MAPPING_KEY = "org.pentaho.reporting.engine.classic.core.modules.misc.datafactory.sql.ColumnNameMapping"; //$NON-NLS-1$
  ...
  public synchronized TableModel queryData(final String query, final DataRow parameters)
    ...
    try {
      ...
      final String translatedQuery = parametrizationProvider.rewriteQueryForParametrization(connection, query,
          parameters);
  ...
  private ParametrizationProviderFactory createParametrizationProviderFactory() throws ReportDataFactoryException {
    ...
    if (parametrizationProviderClassname == null) {
      ...
      factory = ObjectUtilities.loadAndInstantiate(parametrizationProviderClassname, SimpleSQLReportDataFactory.class,
  ...
  protected TableModel parametrizeAndQuery(final DataRow parameters, final String translatedQuery,
    ...
    // TODO 添加分页逻辑
    // 提取公共的获取Statement方法
    Statement statement = gStatement(parameters, translatedQuery, preparedParameterNames);
    ...
    try {
      ...
      // this fails for MySQL as their driver is buggy. We will not add workarounds
      // here, as
      // all drivers are buggy and this is a race we cannot win. Put pressure on the
      // driver
    ...
    // Track the currently running statement - just in case someone needs to cancel
    // it
    final ResultSet res;
    try {
  ...
  public ResultSet performQuery(Statement statement, final String translatedQuery,
      final String[] preparedParameterNames) throws SQLException {
    final ResultSet res;
   ...
   * @param preparedParameterNames
   * @return
   * @throws SQLException
   */
  private Statement gStatement(final DataRow parameters, final String translatedQuery,
      final String[] preparedParameterNames) throws SQLException {
  ...
  public void cancelRunningQuery() {
    ...
    try {
      ...
      logger.warn(
          "Could not cancel running query [maybe the driver does not support that operation] : " + e.getMessage());
    } finally {
